...
defined('BASEPATH') OR exit('Não é permitido esse acesso');
...
class Ordem_servico_model extends CI_Model{
 ...
 public function get_by_id($ordem_servico_id = NULL){
   ...
   public function get_all_servicos_by_ordem($ordem_servico_id = NULL){
      ...
      // Faz a verificação , caso for passado esse id no ordem_servico_id, faz a consulta no banco
   ...
   public function get_all_servicos($ordem_servico_id = NULL){
    ...
    $this->db->select([
      ...
      'FORMAT(SUM(REPLACE(ordem_ts_valor_total,",", "")), 2) as ordem_ts_valor_unitario',
      'servicos.servico_id',
   ...
   public function gerar_relatorio_os($data_inicial = NULL, $data_final = NULL){
    $this->db->select([
     ...
     if($data_inicial && $data_final){ // SE FOI PASSADO A DATA INICIAL E DATA FINAL
